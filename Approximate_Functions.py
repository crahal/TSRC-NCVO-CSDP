...
def approximatematch(dfinputlist,register,fastorfull,base_dir,dicecoefficientcutoff):
    ...
    dffullregister = pd.read_csv(os.path.join(base_dir, filename), header = None,sep='\t',encoding='latin-1',names=['raw_name'])
    ...
    dflist = pd.read_csv(os.path.join(base_dir, filename), header = None,sep='\t',encoding='latin-1',names=['keyword'])
    if fastorfull == 'fast':
        index=0
        start=time.time()
...
#            print('Now on keyword: '+keyword+ ' which is '+register+' word '+str(index)+' of '+str(len(dflist['keyword'].index)))
            try:
                dfinputlist_temp=dfinputlist[dfinputlist['raw_name'].str.contains(keyword)==1]
                dfinputlist_temp['norm_name']=dfinputlist_temp['raw_name'].str.replace(keyword,'')
                dfregister_temp=dffullregister[dffullregister['raw_name'].str.contains(keyword)==True]
                dfregister_temp['norm_name']=dfregister_temp['raw_name'].str.replace(keyword,'')
                mask = (dfregister_temp['norm_name'].astype('str').str.len()>5)
                dfregister_temp=dfregister_temp.loc[mask]
                dfregister_temp = dfregister_temp.reset_index(drop=True)
                tqdm.pandas(desc='Register: '+register+'. Keyword: '+keyword+' ('+str(index)+' of '+str(len(dflist.index))+'). Total of '+str(len(dfinputlist_temp.index))+' to find. Finished')
                dfbestmatches_temp=dfinputlist_temp.progress_apply(applywrapper,args=(dfregister_temp,),axis=1)
                dfbestmatches=dfbestmatches.append(dfbestmatches_temp)
            except:
                pass
        print('Finished '+register+' fastmatch after '+str(time.time()-start)+ ' seconds!')
    else: 
        start=time.time()
        dfbestmatches=dfinputlist.progress_apply(applywrapper,args=(dffullregister,),axis=1)
        print('Finished applying the fullmatch after '+str(time.time()-start)+ ' seconds!')
    dfbestmatches=dfbestmatches[dfbestmatches['bestscore']>=dicecoefficientcutoff]
...
def applywrapper(dfinputlist,dfregister):
    ...
    if len(dfinputlist['norm_name'])>5:
        d=[dice_coefficient(dfinputlist['norm_name'],dfregister_norm['norm_name'][indexx]) for indexx,rows in dfregister_norm.itertuples()]
...
#        return 0.0
    #if a == b: return 1.0
    if abs(len(a)-len(b))>4: return 0.0
